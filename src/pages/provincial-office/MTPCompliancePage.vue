...
import { editMTPProfile, viewMTPProfileByApplicationID } from '../../api/mtpProfile.js';
...
onMounted(async () => {
  if (appId) {
    const data = await viewMTPProfileByApplicationID(appId);
...
const saveProfile = async () => {
  try {
    const response = await editMTPProfile(appId, profile.value);
    const refreshedData = await viewMTPProfileByApplicationID(appId);
...
const generatePrintReport = () => {
    ...
    <head>
      ...
      <style>
        ...
        .label-cell { font-weight: bold; background-color: #fbfbfb; color: #3c4043; width: 25%; }
    ...
    <body>
        <div class="sheet-header-container">
            ...
            <div>
                <h1 class="sheet-title">MTP Application Data - ${profile.value.program_applied}</h1>
